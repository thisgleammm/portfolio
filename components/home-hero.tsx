...
export function HomeHero() {
  ...
  const itemVariants = {
    ...
    visible: {
      ...
      transition: {
        duration: 0.8,
        ease: [0.16, 1, 0.3, 1] as const, // power4.out equivalent
      },
    },
  };
    ...
    <div className="flex min-h-screen w-full flex-col bg-background text-foreground overflow-x-hidden selection:bg-accent selection:text-accent-foreground transition-colors duration-300">
      ...
      >
        <motion.div variants={navVariants}>
          <Link href="/" className="glass capsule flex items-center gap-3 hover:bg-accent-muted transition-colors active:scale-95 group">
        ...
        <motion.div variants={navVariants} className="glass capsule hidden md:flex items-center gap-6">
          {navLinks.map((link) => (
        ...
        <motion.div variants={navVariants} className="flex items-center gap-2 md:gap-3">
          <ThemeToggle />
      ...
      >
        <div className="space-y-4 max-w-4xl flex-1 flex flex-col justify-center">
          <motion.h2 variants={itemVariants} className="text-2xl md:text-4xl lg:text-5xl font-bold tracking-tight text-black dark:text-white">Hi, I&apos;m Gleam 👋</motion.h2>
          <motion.h1 variants={itemVariants} className="text-3xl md:text-6xl lg:text-6xl font-bold text-primary leading-[0.9] tracking-tighter">I build things for the <br className="hidden md:block" /> web and beyond.</motion.h1>
          <motion.p variants={itemVariants} className="mt-8 mx-auto max-w-2xl text-lg md:text-xl text-muted-foreground font-medium">Full-Stack Engineer <span className="mx-2 opacity-30">·</span> Next.js <span className="mx-2 opacity-30">·</span> Laravel <span className="mx-2 opacity-30">·</span> Python</motion.p>
        </div>
        ...
        <motion.div 
          ...
          className="w-full flex flex-col md:flex-row items-center justify-between gap-8 mt-12"
        >
          ...
          <div className="flex flex-col md:flex-row items-center gap-4">
            <motion.div variants={itemVariants} className="hidden lg:block"><MusicWidget /></motion.div>
            <motion.div variants={itemVariants} className="glass capsule flex items-center gap-2 md:gap-4 py-1.5 px-3 md:px-6">
            ...
            <motion.a variants={itemVariants} href="mailto:thisgleam@gmail.com" className="glass capsule flex items-center gap-2 md:gap-3 py-1.5 px-4 md:px-6 hover:bg-accent-muted transition-all active:scale-95 group">
